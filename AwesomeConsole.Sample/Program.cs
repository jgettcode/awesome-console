...
Table.Configure(o => o.NumberAlignment = Alignment.Right)
    .AddColumns("first", "second column", "three")
    .AddRow(1, 2, Math.PI)
...
new Table("Last Name", "First Name", new TableColumn("Elected", Alignment.Right), new TableColumn("Birthday", "{0:MMMM dd, yyyy}"))
...
Console.WriteLine("***** generate from a collection of objects and calculated column");
data.ToConsoleTable().AddColumn("Age When Elected", (x, i) => x.YearElected - x.Birthday.Year).Write();
...
{
    o.Columns[2].HeaderText = "number";
    o.Columns[2].HeaderAlignment = Alignment.Center;
    o.Columns[2].Format = "{0:0.00000000}";
}).Write();
